...
{
    /*
    The plan for this class is to use CustomLightSource.Mesh as a mask
    That way, a sprite object will have two sprites
    One that's lit up, and the other that's dark
    It'll prolly check all CustomLightSources in the scene, might have to have it check by distance for performance
    */
    ...
    {
        Vector2[] ret = new Vector2[mesh.vertices.Length];
        for (int i = 0; i < ret.Length; i++) ret[i] = mesh.vertices[i];
        return ret;
    }

    private static Vector2[] Poly(params Vector2[] args) => args;

    private static IEnumerable<GameObject> GetChildren(GameObject obj)
    {
        List<GameObject> ret = new List<GameObject>();
        foreach (Transform t in obj.transform) ret.Add(t.gameObject);
        return ret;
    }
...
public static class SutherlandHodgman
{
    ...
    private static bool IsNearZero(float testValue) => Math.Abs(testValue) <= .000000001d;
